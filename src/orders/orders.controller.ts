import { Controller, Get, Post, Param, UseGuards, Body, Request, Response, UseInterceptors, UploadedFile, ParseFilePipe, FileTypeValidator, MaxFileSizeValidator, UploadedFiles, Delete, Put } from '@nestjs/common';
...
export class OrdersController {
    ...
    @Post('/')
    @UseInterceptors(FilesInterceptor('fotoHp[]', 5))
    async createOrder(@Request() req, @Response() res, @Body() data: CreateOrderDto,
        @UploadedFiles(
            new ParseFilePipe({
                validators: [
    ...
    @Put('/kt/:id')
    async kepalaTeknisiTakeover(@Request() req, @Response() res, @Param('id') id: string) {
    ...
    async assignTeknisi(@Request() req, @Response() res, @Param('id') id: string,
        @UploadedFiles(
            new ParseFilePipe({
                validators: [
                    new MaxFileSizeValidator({
    ...
    async inProgress(@Request() req, @Response() res, @Param('id') id: string,
        @UploadedFiles(
            new ParseFilePipe({
                validators: [
                    new MaxFileSizeValidator({
    ...
    async finished(@Request() req, @Response() res, @Param('id') id: string,
        @UploadedFiles(
            new ParseFilePipe({
                validators: [
                    new MaxFileSizeValidator({
